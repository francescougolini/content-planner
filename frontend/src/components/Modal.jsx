...
const Modal = forwardRef(function Modal({ children, title, className = '', ...props }, ref) {
  ...
  const getFocusable = (root) => {
    if (!root) return [];
    const nodes = root.querySelectorAll('a[href], area[href], input:not([disabled]):not([type="hidden"]), select:not([disabled]), textarea:not([disabled]), button:not([disabled]), [tabindex]:not([tabindex="-1"])');
  ...
  useImperativeHandle(ref, () => ({
    ...
    close: () => {
      if (!dlgRef.current) return;
      try { dlgRef.current.close?.(); } catch (e) { /* ignore */ }
  ...
  return (
    <dialog
      ...
      {...props}
    >
      {title && <div id={titleId} className="panel-title">{title}</div>}
